...
from squad.utils import load_preprocess_data
...
def metrics(data):
    ...
    story_word_maxlen = max(map(len, (x for x, _, _ in data)))
    query_word_maxlen = max(map(len, (x for _, x, _ in data)))

    return story_word_maxlen, query_word_maxlen
...
def vectorize_stories(data_char, char_idx, word_idx):
    ...
    for story_char, question_char, answer in data_char:
        ...
        indices_word = [[k, i] for i, o in enumerate(x) if o == char_idx[" "]]
        indices_sentence = [[k, indices_word.index([k, i-1])] for i, o in enumerate(x)
                            if o == char_idx["."] and [k, i-1] in indices_word]
        X.append(x)
        ...
        qindices_word = [[k, i] for i, o in enumerate(xq) if o == char_idx[" "]]
        Xq.append(xq)
        qY_length.append(len(qindices_word))
        qindices_word += [[k, 0]] * (query_word_maxlen - len(qindices_word))
...
if __name__ == "__main__":
    ...
    vocab_char = sorted(reduce(lambda x, y: x | y, (set(story_char + question_char)
                                                    ...
                                                    train_data + val_data)))
    vocab_word = sorted(set(answer for story_char, question_char, answer in train_data + val_data))
    ...
    story_char_maxlen = max(map(len, (x for x, _, _ in train_data + val_data)))
    query_char_maxlen = max(map(len, (x for _, x, _ in train_data + val_data)))

    train_data_word, val_data_word = load_preprocess_data(path, mode_story="word", size=config.size_dataset)
    story_word_maxlen, query_word_maxlen \
        = max(metrics(train_data_word), metrics(val_data_word))

    X, Xq, Y, X_length, Y_length, Indices_word, \
    Indices_sentence_tmp, qX_length, qY_length, qIndices_word = vectorize_stories(train_data, char_idx, word_idx)
    tX, tXq, tY, tX_length, tY_length, tIndices_word, \
    tIndices_sentence_tmp, tqX_length, tqY_length, tqIndices_word = vectorize_stories(val_data, char_idx, word_idx)

    Indices_sentence = []
    maxsteps_train = max(len(l) for l in Indices_sentence_tmp)
    tIndices_sentence = []
    maxsteps_val = max(len(l) for l in tIndices_sentence_tmp)
    story_maxsteps = max(maxsteps_train, maxsteps_val)
    k = 0
    for sentence in Indices_sentence_tmp:
        tmp = sentence
        tmp += [[k,0]] * (story_maxsteps - len(tmp))
        Indices_sentence.append(tmp)
        k += 1
    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.Session() as session:
            ...
            model_dir = os.path.join("logs_squad/", str(timestamp + "_e" + str(config.embed_size)))
